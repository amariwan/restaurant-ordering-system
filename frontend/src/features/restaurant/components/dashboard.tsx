...
import { IconClipboard, IconClock, IconCircleCheck, IconGrillFork, IconUsers, IconTrendingUp } from '@tabler/icons-react';
import Link from 'next/link';
import { ordersAllOptions, usersAllOptions, tablesAllOptions } from '@/features/restaurant/api/queries';
    ...
    <div className="space-y-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{t.admin.ordersPlacedToday.replace('{count}', String(todayOrders))}</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex items-center gap-2 text-sm">
            {todayOrders > 0 ? (
              <><IconTrendingUp className="w-4 h-4 text-green-600" /><span className="text-green-600">{t.admin.active}</span></>
            ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-1 text-sm">
            {orders.items.slice(-5).toReversed().map((o) => (
              <Link key={o.id} href={`/orders/${o.id}`} className="flex items-center justify-between gap-2 py-2 border-b last:border-b-0 hover:bg-muted/50 -mx-2 px-2 rounded transition-colors">
                <div>
                ...
                <Badge variant={o.status === 'cancelled' ? 'destructive' : o.status === 'ready' ? 'secondary' : 'secondary'}>
                  {o.status}
                </Badge>
              </Link>
            ))}
            {orders.items.length === 0 && <div className="text-sm text-muted-foreground py-4 text-center">{t.admin.noOrdersYet}</div>}
          </CardContent>
        </Card>
      </div>
    </div>
