...
import { menuCategoriesOptions, menuItemsOptions, useMenuDeleteCategoryMutation, useMenuDeleteItemMutation } from '../api/queries';
    ...
    <div className='space-y-6'>
      <section>
        <div className='flex items-center justify-between mb-4'>
          ...
          <div className='flex items-center gap-2'>
            <Button variant='outline' onClick={() => { setEditingCategory(undefined); setCatSheetOpen(true); }}>{t.menu.addCategory}</Button>
          </div>
        </div>
              ...
              <TableRow key={c.id}>
                ...
                <TableCell>
                  <div className='flex gap-2'>
                    <Button variant='ghost' size='sm' onClick={() => { setEditingCategory(c); setCatSheetOpen(true); }}>
                      <IconPencil />
                    </Button>
      ...
      <section>
        <div className='flex items-center justify-between mb-4'>
          ...
          <div className='flex items-center gap-2'>
            <Button variant='outline' onClick={() => { setEditingItem(undefined); setItemSheetOpen(true); }}>{t.menu.addItem}</Button>
          </div>
        </div>
          ...
          <TableBody>
            {items.items.map((it) => (
              <TableRow key={it.id}>
                ...
                <TableCell>
                  {it.imageUrl ? (
                    <img src={it.imageUrl} alt={localizedValue(it, 'name', locale)} className='h-12 w-20 object-cover rounded-md' />
                  ) : (
                    <div className='h-12 w-20 flex items-center justify-center bg-muted rounded-md'><IconPhoto /></div>
                  )}
                </TableCell>
                ...
                <TableCell>
                  <div className='flex gap-2'>
                    <Button variant='ghost' size='sm' onClick={() => { setEditingItem(it); setItemSheetOpen(true); }}>
                      <IconPencil />
                    </Button>
                    <Button variant='destructive' size='sm' onClick={async () => {
                      try {
      ...
      {/* Sheets */}
      {/** Category sheet **/}
      <CategoryFormSheet category={editingCategory} open={catSheetOpen} onOpenChange={setCatSheetOpen} />
