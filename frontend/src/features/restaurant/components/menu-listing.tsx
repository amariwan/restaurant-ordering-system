...
import { menuItemsOptions, menuCategoriesOptions, useOrdersAddItemMutation } from '@/features/restaurant/api/queries';
import { useCartStore } from '@/features/restaurant/lib/cart-store';
import { useI18n } from '@/lib/i18n/context';
import { localizedValue } from '@/lib/i18n/helpers';

export function MenuListing({ categoryId: initialCategoryId, orderId, tableId: initialTableId }: { categoryId?: number; orderId?: number; tableId?: number }) {
  const { data: categories } = useSuspenseQuery(menuCategoriesOptions);
  const { data: items } = useSuspenseQuery(menuItemsOptions(initialCategoryId ? { categoryId: initialCategoryId } : undefined));
          ...
          <Card key={item.id} className="overflow-hidden hover:shadow-md transition-shadow">
            ...
            <CardContent className="space-y-3">
              <div>
                ...
                <div className="text-sm text-muted-foreground">{localizedValue(item, 'categoryName', locale)}</div>
                {localizedValue(item, 'description', locale) && (
                  <p className="text-xs text-muted-foreground mt-1">{localizedValue(item, 'description', locale)}</p>
                )}
              </div>
                  ...
                  onClick={() => {
                    if (orderId) {
                      addToOrder.mutate(
                        ...
                        { onSuccess: () => toast.success(`${localizedValue(item, 'name', locale)} ${t.menu.addedToOrder} #${orderId}`), onError: () => toast.error(t.common.failed) }
                      );
                    } else {
                      addItemToCart({ menuItemId: item.id, menuItemName: item.nameEn, menuItemNameKu: item.nameKu, price: item.price, quantity: 1 });
