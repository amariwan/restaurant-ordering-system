    ...
    try {
      if (isEdit && reservation) {
        await updateMutation.mutateAsync({
          id: reservation.id,
          data: { customerName: customerName.trim(), customerEmail: customerEmail.trim(), customerPhone: customerPhone.trim() || undefined, guestCount, reservationTime: new Date(reservationTime).toISOString(), note: note.trim() || undefined },
        });
      ...
      toast.error(err?.message ?? (t.reservations.failedSave ?? 'Failed to save reservation'));
    }
  }
      ...
      <SheetContent side="right">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 pt-4">
          ...
          <div className="grid gap-2">
            ...
            <Input id="guests" type="number" min={1} value={guestCount} onChange={(e) => setGuestCount(Number(e.target.value))} required />
          </div>
          <div className="grid gap-2">
            <Label htmlFor="time">{t.reservations.dateTime}</Label>
            <Input id="time" type="datetime-local" value={reservationTime} onChange={(e) => setReservationTime(e.target.value)} required />
          </div>
          <div className="grid gap-2">
          ...
          <SheetFooter className="pt-4">
            <Button variant="outline" onClick={() => onOpenChange(false)} type="button">{t.common.cancel}</Button>
