...
export const menuItemSchema = z.object({
  ...
  categoryId: z.union([z.number(), z.literal('')]).refine((val) => typeof val === 'number', 'Category is required'),
});
...
export const tableSchema = z.object({
  number: z.string().transform((val) => parseInt(val, 10)).refine((val) => !isNaN(val) && val > 0, 'Table number must be a positive number'),
});
