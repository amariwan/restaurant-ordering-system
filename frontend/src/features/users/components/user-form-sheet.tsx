...
import type { CreateUserPayload } from '../api/service';
...
export function UserFormSheet({ user, open, onOpenChange }: UserFormSheetProps) {
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      if (isEdit) {
        await updateMutation.mutateAsync({ id: user!.id, values: userPayload });
      } else {
        if (!value.password || value.password.length < 8) {
          toast.error(t.userFormSheet.pwError);
          return;
        }
            ...
            <form.Form id='user-form-sheet' className='space-y-4'>
              ...
              {!isEdit && (
                <FormTextField
                  ...
                  validators={{
                    onBlur: z.string().min(8, t.userFormSheet.pwError)
                  }}
                />
              )}
